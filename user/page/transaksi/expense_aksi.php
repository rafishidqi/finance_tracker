...
if (isset($_POST['tomboltambah'])) {
    ...
    $type = 'expense'; // Karena ini adalah form expense
    $foto_path = null;
    ...
    if (mysqli_query($varkoneksi, $query)) {
        // Jika transaksi berhasil, kurangi saldo wallet
        ...
        $summary_query = "
            ...
            VALUES ('$user_id', '$date', 0, $amount, -$amount, '$created_at')
            ON DUPLICATE KEY UPDATE 
                total_expense = total_expense + $amount,
                net_balance = total_income - total_expense"; // Pastikan net_balance dihitung dengan benar

        // Eksekusi query summary
        if (mysqli_query($varkoneksi, $summary_query)) {
            // Redirect ke halaman transaksi setelah berhasil
            echo "<script>
            Swal.fire({
                icon: 'success',
                title: 'Success!',
                text: 'Add Data Expense Successful!'
            }).then(() => {
                window.location.href='index.php?halaman=transaksi';
            });
            </script>";
        } else {
            // Jika gagal update summary
            echo "<script>
                    Swal.fire({
                        icon: 'error',
                        title: 'Error!',
                        text: 'Something Wrong while updating summary: " . mysqli_error($varkoneksi) . "'
                    }).then(() => {
                        window.history.back();
                    });
                </script>";
